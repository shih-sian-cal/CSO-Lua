local playerID = {};
local verify_id = nil;
local syncvar_get = G.SV.Create( ClientID.SYNCVAR.GET );
local syncvar_get_all = G.SV.Create( ClientID.SYNCVAR.GET_ALL );
local syncvar_verify = G.SV.Create( ClientID.SYNCVAR.VERIFY );
local syncvar_current_time = G.SV.Create( ClientID.SYNCVAR.CURRENT_TIME );
