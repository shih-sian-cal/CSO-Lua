...
local syncvar_verify = UI.SyncValue.Create( ClientID.SYNCVAR.VERIFY );
local syncvar_current_time = UI.SyncValue.Create( ClientID.SYNCVAR.CURRENT_TIME );
...
function ClientID:Verify_OnSync()
    ...
    if verifyDelay ~= nil and currentTime ~= nil and verifyDelay <= currentTime
    then
